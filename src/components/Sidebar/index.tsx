...
import { Search, Menu, X, MapPin, Calendar, ChevronRight, Download, Upload } from 'lucide-react';
...
const Sidebar: React.FC<SidebarProps> = ({ 
  ...
  return (
    <div className={`bg-card border-r border-border h-screen transition-all duration-300 ease-in-out overflow-hidden ${isCollapsed ? 'w-16' : 'w-64'}`}>
      {/* Sidebar Header */}
      ...
      <div className="h-[calc(100vh-180px)] overflow-y-auto">
        {!isCollapsed ? (
          <div className="p-4">
            <h2 className="text-lg font-semibold mb-4">我的足迹</h2>
            {/* 统计小组件 */}
          ...
          <div className="p-2">
            {filteredFootprints.map((footprint) => (
              <div
                key={footprint.id}
                className={`p-3 rounded-md cursor-pointer transition-all mb-2 flex items-center justify-center ${selectedFootprintId === footprint.id ? 'bg-primary text-primary-foreground' : 'bg-background hover:bg-accent'}`}
                onClick={() => onSelectFootprint(footprint)}
